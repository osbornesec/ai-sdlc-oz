...
try:
    ...
    ActualAPIAuthError = openai.APIAuthenticationError
...
try:
    ...
    ActualAnthropicAPIAuthError = actual_anthropic_lib.APIAuthenticationError
    ...
    ActualAnthropicAPIError = actual_anthropic_lib.APIError
except ImportError:
    ...
    ActualAnthropicAPIError = Exception
...
class MockAnthropicModule:
    ...
    class APIStatusError(Exception):
        def __init__(self, message, *, response, body=None):
            ...
            self.status_code = getattr(response, 'status_code', 500)
            self.body = body
    class APIError(Exception): pass # Base error
...
# We patch 'ai_sdlc.services.ai_service.openai' which is where the 'openai' module is imported.
@patch("ai_sdlc.services.ai_service.openai", new_callable=lambda: MockOpenAIModule())
def test_generate_text_openai_success(mock_openai_module, openai_provider_config: AiProviderConfig):
    # `mock_openai_module` is now an instance of MockOpenAIModule.
    # `openai.OpenAI` in the function under test will refer to `MockOpenAIModule.OpenAI`.
    # We need to make sure that when `MockOpenAIModule.OpenAI(api_key=...)` is called,
    # its `chat.completions.create` method is a mock we can control.

    mock_openai_client_instance = MagicMock(spec=MockOpenAIModule.OpenAI) # Mock the client instance
...
@patch("ai_sdlc.services.ai_service.openai", new_callable=lambda: MockOpenAIModule())
def test_generate_text_openai_empty_content(mock_openai_module, openai_provider_config: AiProviderConfig):
    mock_openai_client_instance = MagicMock(spec=MockOpenAIModule.OpenAI)
    mock_chat_completion = MagicMock()
    mock_chat_completion.message = MagicMock()
...
@pytest.mark.parametrize(
    ...
    [
        ("APIAuthenticationError", "OpenAI API Authentication Error"),
...
def test_generate_text_openai_standard_exceptions(
    mock_openai_module, openai_provider_config: AiProviderConfig,
...
@patch("ai_sdlc.services.ai_service.openai", new_callable=lambda: MockOpenAIModule())
def test_generate_text_openai_apistatuserror(mock_openai_module, openai_provider_config: AiProviderConfig):
...
@patch("ai_sdlc.services.ai_service.openai", new_callable=lambda: MockOpenAIModule())
def test_generate_text_openai_unexpected_exception(mock_openai_module, openai_provider_config: AiProviderConfig):
    mock_openai_client_instance = MagicMock(spec=MockOpenAIModule.OpenAI)
...
@patch("ai_sdlc.services.ai_service.anthropic", new_callable=lambda: MockAnthropicModule())
def test_generate_text_anthropic_success(mock_anthropic_module, anthropic_provider_config: AiProviderConfig):
    mock_anthropic_client_instance = MagicMock(spec=MockAnthropicModule.Anthropic)
...
@patch("ai_sdlc.services.ai_service.anthropic", new_callable=lambda: MockAnthropicModule())
def test_generate_text_anthropic_empty_content(mock_anthropic_module, anthropic_provider_config: AiProviderConfig):
    mock_anthropic_client_instance = MagicMock(spec=MockAnthropicModule.Anthropic)
    mock_message_response = MagicMock()
    mock_text_block = MagicMock()
...
@patch("ai_sdlc.services.ai_service.anthropic", new_callable=lambda: MockAnthropicModule())
def test_generate_text_anthropic_unexpected_content_structure(mock_anthropic_module, anthropic_provider_config: AiProviderConfig):
    mock_anthropic_client_instance = MagicMock(spec=MockAnthropicModule.Anthropic)
...
@pytest.mark.parametrize(
    ...
    [
        ("APIAuthenticationError", "Anthropic API Authentication Error"),
        ...
        ("APIError", "An Anthropic API error occurred"), # Test base APIError
    ]
)
@patch("ai_sdlc.services.ai_service.anthropic", new_callable=lambda: MockAnthropicModule())
def test_generate_text_anthropic_standard_exceptions(
    mock_anthropic_module, anthropic_provider_config: AiProviderConfig,
...
@patch("ai_sdlc.services.ai_service.anthropic", new_callable=lambda: MockAnthropicModule())
def test_generate_text_anthropic_apistatuserror(mock_anthropic_module, anthropic_provider_config: AiProviderConfig):
    mock_api_response = MagicMock()
...
@patch("ai_sdlc.services.ai_service.anthropic", new_callable=lambda: MockAnthropicModule())
def test_generate_text_anthropic_unexpected_exception(mock_anthropic_module, anthropic_provider_config: AiProviderConfig):
    mock_anthropic_client_instance = MagicMock(spec=MockAnthropicModule.Anthropic)
...
@patch("ai_sdlc.services.ai_service.get_api_key")
def test_generate_text_missing_model_for_api_provider(mock_get_api_key: MagicMock, openai_provider_config: AiProviderConfig, anthropic_provider_config: AiProviderConfig):
    mock_get_api_key.return_value = "retrieved_api_key"
...
def test_generate_text_openai_apistatuserror_response_missing_status_code(
    ...
    with pytest.raises(OpenAIError, match=r"OpenAI API Error \(Status 500\): Error with response lacking status_code"):
        generate_text_openai(
            "A prompt",
            openai_provider_config["model"],
            "a_key",
