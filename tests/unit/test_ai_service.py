...
def test_get_api_key_missing_config_var_key(openai_provider_config: AiProviderConfig):
    ...
    with pytest.raises(
        ApiKeyMissingError, match="Configuration error: 'api_key_env_var' is not set"
    ):
...
def test_get_api_key_env_var_not_set(openai_provider_config: AiProviderConfig):
    ...
    with pytest.raises(
        ApiKeyMissingError,
        match=f"API key not found: Environment variable {env_var_name} is not set",
    ):
...
def test_generate_text_openai_empty_content(openai_provider_config: AiProviderConfig):
    ...
    with patch.dict("sys.modules", {"openai": mock_openai_module}):
        ...
        with pytest.raises(
            OpenAIError, match="OpenAI API returned an empty message content."
        ):
            generate_text_openai(
                "Prompt for empty",
...
def test_generate_text_openai_standard_exceptions(
    ...
    openai_exception_class_name: str,
    expected_message_snippet: str,
):
    mock_openai_module = MockOpenAIModule()
    ...
    with patch.dict("sys.modules", {"openai": mock_openai_module}):
        ...
        mock_openai_client_instance.chat.completions.create.side_effect = (
            ExceptionToRaise("Mocked API error details")
        )
        mock_openai_module.OpenAI = MagicMock(return_value=mock_openai_client_instance)
...
def test_generate_text_openai_unexpected_exception(
    openai_provider_config: AiProviderConfig,
):
    mock_openai_module = MockOpenAIModule()
    ...
    with patch.dict("sys.modules", {"openai": mock_openai_module}):
        mock_openai_client_instance = MagicMock()
        mock_openai_client_instance.chat.completions.create.side_effect = ValueError(
        ...
        with pytest.raises(
            OpenAIError,
            match="An unexpected error occurred with OpenAI: Completely unexpected error",
        ):
            generate_text_openai(
                "A prompt",
                openai_provider_config["model"],
...
def test_generate_text_anthropic_success(anthropic_provider_config: AiProviderConfig):
    ...
    with patch.dict("sys.modules", {"anthropic": mock_anthropic_module}):
        ...
        mock_anthropic_client_instance.messages.create.return_value = (
            mock_message_response
        )
        mock_anthropic_module.Anthropic = MagicMock(
            return_value=mock_anthropic_client_instance
        )
        ...
        mock_anthropic_module.Anthropic.assert_called_once_with(
            api_key="an_anthropic_key",
            timeout=float(anthropic_provider_config["timeout_seconds"]),
        )
...
def test_generate_text_anthropic_empty_content(
    ...
    with patch.dict("sys.modules", {"anthropic": mock_anthropic_module}):
        ...
        with pytest.raises(
            AnthropicError, match="Anthropic API returned empty message content."
        ):
            generate_text_anthropic(
                "Prompt", anthropic_provider_config["model"], "key", 60
            )


def test_generate_text_anthropic_unexpected_content_structure(
    anthropic_provider_config: AiProviderConfig,
):
    mock_anthropic_module = MockAnthropicModule()
    ...
    with patch.dict("sys.modules", {"anthropic": mock_anthropic_module}):
        ...
        mock_anthropic_client_instance.messages.create.return_value = (
            mock_message_response_empty_content_list
        )
        mock_anthropic_module.Anthropic = MagicMock(
            return_value=mock_anthropic_client_instance
        )
        ...
        with pytest.raises(
            AnthropicError,
            match="Anthropic API returned unexpected or empty content structure.",
        ):
            generate_text_anthropic(
                "Prompt", anthropic_provider_config["model"], "key", 60
            )
        ...
        mock_anthropic_client_instance.messages.create.return_value = (
            mock_message_response_no_text_attr
        )
        with pytest.raises(
            AnthropicError,
            match="Anthropic API returned unexpected or empty content structure.",
...
def test_generate_text_anthropic_standard_exceptions(
    anthropic_provider_config: AiProviderConfig,
    anthropic_exception_class_name: str,
    expected_message_snippet: str,
):
    mock_anthropic_module = MockAnthropicModule()

    with patch.dict("sys.modules", {"anthropic": mock_anthropic_module}):
        ExceptionToRaise = getattr(
            mock_anthropic_module, anthropic_exception_class_name
        )

        mock_anthropic_client_instance = MagicMock()
        mock_anthropic_client_instance.messages.create.side_effect = ExceptionToRaise(
            "Mocked Anthropic API error"
        )
        mock_anthropic_module.Anthropic = MagicMock(
            return_value=mock_anthropic_client_instance
        )
...
def test_generate_text_anthropic_apistatuserror(
    anthropic_provider_config: AiProviderConfig,
):
    mock_anthropic_module = MockAnthropicModule()
    ...
    with patch.dict("sys.modules", {"anthropic": mock_anthropic_module}):
        ...
        mock_anthropic_client_instance = MagicMock()
        mock_anthropic_client_instance.messages.create.side_effect = (
            status_error_instance
        )
        mock_anthropic_module.Anthropic = MagicMock(
            return_value=mock_anthropic_client_instance
        ...
        with pytest.raises(
            AnthropicError, match=r"Anthropic API Error \(Status 401\):"
        ):
            generate_text_anthropic(
                "Prompt", anthropic_provider_config["model"], "key", 60
            )
...
def test_generate_text_anthropic_unexpected_exception(
    anthropic_provider_config: AiProviderConfig,
):
    mock_anthropic_module = MockAnthropicModule()
    ...
    with patch.dict("sys.modules", {"anthropic": mock_anthropic_module}):
        mock_anthropic_client_instance = MagicMock()
        mock_anthropic_client_instance.messages.create.side_effect = TypeError(
            "Unexpected TypeError from Anthropic client"
        )
        mock_anthropic_module.Anthropic = MagicMock(
            return_value=mock_anthropic_client_instance
        )
        ...
        with pytest.raises(
            AnthropicError,
            match="An unexpected error occurred with Anthropic: Unexpected TypeError",
        ):
            generate_text_anthropic(
                "Prompt", anthropic_provider_config["model"], "key", 60
            )
...
def test_generate_text_anthropic_custom_max_tokens(
    anthropic_provider_config: AiProviderConfig,
):
    mock_anthropic_module = MockAnthropicModule()
...
def test_generate_text_openai_negative_timeout():
    with pytest.raises(
        AiServiceError, match="timeout_seconds must be a positive integer"
    ):
...
def test_generate_text_manual_provider_behavior(
...
@patch("ai_sdlc.services.ai_service.get_api_key")
def test_generate_text_unsupported_provider_error(
    mock_get_api_key: MagicMock, custom_provider_config: AiProviderConfig
):
    mock_get_api_key.return_value = "custom_key_value"

    with pytest.raises(
        UnsupportedProviderError,
        match="Unsupported AI provider: custom_provider. Supported providers are: 'openai', 'anthropic', 'manual'.",
    ):
...
def test_generate_text_missing_model_for_api_provider(
    openai_provider_config: AiProviderConfig,
    anthropic_provider_config: AiProviderConfig,
):
    # Test for OpenAI
    ...
    with pytest.raises(
        AiServiceError,
        match="Configuration error: 'model' is not set for provider 'openai'",
    ):
    ...
    with pytest.raises(
        AiServiceError,
        match="Configuration error: 'model' is not set for provider 'anthropic'",
    ):
...
def test_generate_text_api_key_error_propagates(
    openai_provider_config: AiProviderConfig,
):
    ...
    with pytest.raises(
        ApiKeyMissingError
    ):  # The specific message is tested in get_api_key tests
...
@patch("ai_sdlc.services.ai_service.get_api_key")
def test_generate_text_manual_provider_no_api_key_call(
    mock_get_api_key: MagicMock, manual_provider_config: AiProviderConfig
):
...
def test_provider_config_includes_direct_api_calls(
    openai_provider_config: AiProviderConfig,
):
...
def test_generate_text_openai_apistatuserror_response_missing_status_code(
    ...
    with patch.dict("sys.modules", {"openai": mock_openai_module}):
        mock_response_no_status = MagicMock(
            spec=[]
        )  # No attributes, so getattr(response, 'status_code', default) will use default.
