...
class TestNextCommand:
    ...
    def test_validate_required_files_missing_prev_file(self, temp_project_dir: Path, capsys):
        ...
        with pytest.raises(SystemExit) as exc_info:
            next_cmd._validate_required_files(prev_file, prompt_file, '00-idea', '01-prd', {})
    ...
    def test_validate_required_files_missing_prompt_file(self, temp_project_dir: Path, capsys):
        ...
        with pytest.raises(SystemExit) as exc_info:
            next_cmd._validate_required_files(prev_file, prompt_file, '00-idea', '01-prd', {'prompt_dir': 'prompts'})
        assert exc_info.value.code == 1
    ...
    def test_prepare_file_paths(self, temp_project_dir: Path):
        ...
        with patch('ai_sdlc.commands.next.ROOT', temp_project_dir):
            workdir, prev_file, prompt_file, next_file, prompt_output = next_cmd._prepare_file_paths(
                conf, 'test-feature', '00-idea', '01-prd'
            )
