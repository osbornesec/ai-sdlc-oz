...
@pytest.fixture
def mock_ai_provider_openai_direct(openai_provider_config: AiProviderConfig) -> AiProviderConfig:
...
@pytest.fixture
def mock_ai_provider_openai_direct_no_key(mock_ai_provider_openai_direct: AiProviderConfig) -> AiProviderConfig:
...
@pytest.fixture
def mock_ai_provider_direct_disabled(mock_ai_provider_openai_direct: AiProviderConfig) -> AiProviderConfig:
    config = mock_ai_provider_openai_direct.copy()
...
@pytest.fixture
def setup_working_directory(tmp_path: Path, mock_config: ConfigDict, mock_lock: LockDict):
    # Create directories
    (tmp_path / mock_config["active_dir"] / mock_lock["slug"]).mkdir(parents=True, exist_ok=True)
    (tmp_path / mock_config["prompt_dir"]).mkdir(parents=True, exist_ok=True)

    # Create previous step file
    prev_file_path = tmp_path / mock_config["active_dir"] / mock_lock["slug"] / f"{DEFAULT_CURRENT_STEP}-{mock_lock['slug']}.md"
    prev_file_path.write_text(PREV_STEP_CONTENT)

    # Create prompt template file
    prompt_template_path = tmp_path / mock_config["prompt_dir"] / f"{DEFAULT_NEXT_STEP}.prompt.yml"
...
def auto_mock_dependencies(request): # `request` allows conditional mocking if needed
    ...
    mock_context7 = patch("ai_sdlc.commands.next._apply_context7_enrichment", side_effect=lambda conf, prompt, *args: prompt).start()
...
@pytest.mark.parametrize("mock_config", [pytest.lazy_fixture("mock_ai_provider_openai_direct")], indirect=True)
def test_run_next_direct_api_call_success(
    mock_config: ConfigDict, mock_lock: LockDict,
    setup_working_directory: Path, auto_mock_dependencies: dict,
    capsys: pytest.CaptureFixture
):
    root_path = setup_working_directory
    auto_mock_dependencies["utils_ROOT"].return_value = root_path # Critical for path resolution
    ...
    with patch.dict(os.environ, {mock_config["ai_provider"]["api_key_env_var"]: "fake_key_for_test"}):
        run_next()
    ...
    next_step_file = root_path / mock_config["active_dir"] / mock_lock["slug"] / f"{DEFAULT_NEXT_STEP}-{mock_lock['slug']}.md"
    assert next_step_file.exists()
    assert next_step_file.read_text() == "AI generated content successfully."

    prompt_output_file = root_path / mock_config["active_dir"] / mock_lock["slug"] / f"_prompt-{DEFAULT_NEXT_STEP}.md"
    ...
    auto_mock_dependencies["write_lock"].assert_called_once_with(
        {"slug": mock_lock["slug"], "current": DEFAULT_NEXT_STEP, "created": mock_lock["created"]}
    )
...
@pytest.mark.parametrize("mock_config", [pytest.lazy_fixture("mock_ai_provider_openai_direct")], indirect=True)
@pytest.mark.parametrize("error_to_raise, error_name_in_output", [
    (ApiKeyMissingError("Test API Key Missing"), "API Key Missing Error"),
    (UnsupportedProviderError("Test Unsupported Provider"), "Unsupported Provider Error"),
    (OpenAIError("Test OpenAI Error"), "OpenAI API Error"),
    (AiServiceError("Test Generic AI Service Error"), "AI Service Error"),
    (Exception("Test Unexpected Error during API call"), "An unexpected error occurred"),
])
def test_run_next_direct_api_call_errors_fallback_to_manual(
    mock_config: ConfigDict, mock_lock: LockDict,
    setup_working_directory: Path, auto_mock_dependencies: dict,
    capsys: pytest.CaptureFixture,
    error_to_raise: Exception, error_name_in_output: str
):
    root_path = setup_working_directory
    ...
    with patch.dict(os.environ, {env_var_name: "fake_key_for_test"} if not isinstance(error_to_raise, ApiKeyMissingError) else {}):
    ...
    expected_prompt_content = PROMPT_TEMPLATE_CONTENT.replace(PLACEHOLDER, PREV_STEP_CONTENT)
...
@pytest.mark.parametrize("mock_config", [pytest.lazy_fixture("mock_ai_provider_direct_disabled")], indirect=True)
def test_run_next_direct_api_calls_disabled(
    ...
    captured = capsys.readouterr()
    assert "ℹ️  Direct API calls are disabled or provider is not configured for direct calls." in captured.out
...
@pytest.mark.parametrize("mock_config", [pytest.lazy_fixture("mock_ai_provider_manual")], indirect=True)
...
@pytest.mark.parametrize("mock_config", [pytest.lazy_fixture("mock_ai_provider_manual")], indirect=True) # Config doesn't matter much here
def test_run_next_step_file_already_exists(
    ...
    next_step_file_path = root_path / mock_config["active_dir"] / mock_lock["slug"] / f"{DEFAULT_NEXT_STEP}-{mock_lock['slug']}.md"
    ...
    assert "Generated AI prompt file" not in captured.out # The _write_prompt_and_show_instructions is skipped

    assert "✅  Found existing file:" in captured.out
    assert "✅  Advanced to step: " + DEFAULT_NEXT_STEP in captured.out
    assert "🧹  Cleaned up prompt file:" in captured.out # _prompt file should be cleaned
...
def test_run_next_missing_prev_file(
    ...
    prev_file_path = root_path / mock_config["active_dir"] / mock_lock["slug"] / f"{DEFAULT_CURRENT_STEP}-{mock_lock['slug']}.md"
    ...
    captured = capsys.readouterr()
    assert f"❌ Error: The previous step's output file '{prev_file_path}' is missing." in captured.out
...
def test_run_next_missing_prompt_template_file(
    ...
    prompt_template_path = root_path / mock_config["prompt_dir"] / f"{DEFAULT_NEXT_STEP}.prompt.yml"
    ...
    captured = capsys.readouterr()
    assert f"❌ Critical Error: Prompt template file '{prompt_template_path}' is missing." in captured.out
    mock_exit.assert_called_once_with(1)
    patch.stopall() # Stop sys.exit mock


# Test that if ai_provider_config is missing entirely from config, it defaults to manual-like behavior
@pytest.mark.parametrize("mock_config", [pytest.lazy_fixture("mock_ai_provider_manual")], indirect=True) # Start with a base
def test_run_next_missing_ai_provider_section_in_config(
    ...
    assert "Attempting to generate text" not in captured.out
    assert "📝  Generated AI prompt file:" in captured.out # Falls back to manual prompt generation

    mock_generate_text_func.assert_not_called()

    prompt_output_file = root_path / config_no_ai_section["active_dir"] / mock_lock["slug"] / f"_prompt-{DEFAULT_NEXT_STEP}.md"
    assert prompt_output_file.exists()
    expected_prompt_content = PROMPT_TEMPLATE_CONTENT.replace(PLACEHOLDER, PREV_STEP_CONTENT)
    assert prompt_output_file.read_text() == expected_prompt_content
